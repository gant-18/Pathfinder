...
class LIFNodesLinearDecay(Nodes):
    def __init__(
        self,
        n: Optional[int] = None,
        shape: Optional[Iterable[int]] = None,
        ...
        **kwargs,
        ...
        self.linear_decay = torch.tensor(linear_decay, dtype=torch.float)
        self.lbound = torch.tensor(lbound, dtype=torch.float) if lbound is not None else None

    def forward(self, x: torch.Tensor) -> None:
        # Apply linear decay toward rest
        decay_mask = self.refrac_count <= 0
        self.v -= self.linear_decay * self.dt * decay_mask.float()

        # Add input
        self.v += decay_mask.float() * x

        # Refractory and spike logic
        self.refrac_count -= self.dt
        self.s = self.v >= self.thresh
        self.refrac_count.masked_fill_(self.s, self.refrac)
        self.v.masked_fill_(self.s, self.reset)

        # Clip voltage to lower bound
        if self.lbound is not None:
